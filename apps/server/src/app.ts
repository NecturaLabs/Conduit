...
export async function buildApp(options: BuildAppOptions = {}): Promise<FastifyInstance> {
  ...
  // /.well-known/assetlinks.json — served publicly for Android App Links verification.
  // Android verifies domain ownership by fetching this file at install time.
  // The file is generated by CI from the release keystore and committed to the repo.
  const __dirname = dirname(fileURLToPath(import.meta.url));
  const assetLinksPath = join(__dirname, 'well-known', 'assetlinks.json');
  fastify.get('/.well-known/assetlinks.json', async (_request, reply) => {
    if (!existsSync(assetLinksPath)) {
      return reply.code(404).send({ error: 'Not Found', message: 'assetlinks.json not yet generated', statusCode: 404 });
    }
    const content = readFileSync(assetLinksPath, 'utf8');
    return reply
      .code(200)
